...
推荐在熟悉了 [摇树(Treeshaking)](/features/treeshaking/) 过程之后再继续阅读本章节。
...
`configFile` will be resolved relative to the project root, and loaded before each of vuetify's stylesheets. If you were using the basic technique from above, make sure to either:
